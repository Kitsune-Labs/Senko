module.exports = (message, embeds, emojis, timeout) => {
    if (!message || !message.channel || !message.channel.send || !message.author && !message.user) return {
        error: true,
        message: "Invalid message was provided, It do not have either channel / user object attached to it"
    };
    ...
    if (!emojis || !Array.isArray(emojis) || emojis.length < 2) return { error: true, message: "Please either provide two emojis or don't provide anything at all" };
